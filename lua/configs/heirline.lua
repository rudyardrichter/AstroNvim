...
if status_ok then
  ...
  heirline.setup(
    astronvim.user_plugin_opts("plugins.heirline", {
      hl = { fg = "fg", bg = "bg" },
    ...
    }),
    {
      ...
      { -- active winbar
        ...
        { provider = st.provider.breadcrumbs(), condition = st.condition.aerial_available },
      },
      { -- fallback if not active
        hl = { fg = "winbarnc_fg", bg = "winbarnc_bg" },
        { provider = st.provider.fileicon { padding = { right = 1 } } },
